...
class CXRMultimodalv2(BaseDataset):
    def __init__(self,
                 data_dir: str,
                 split: str,
                 transforms: dict,
        ...
        if self.use_kd_get_item:
            self.collate_fn = multimodal_collate_fn_v3 # for KD
        else:
    ...
    def get_caption(self, report):
        ...
        if self.empty_fu_sent_dummy:
            report = "These are chest radiographs, and it is necessary to review whether the patient's disease progress has improved or worsened."
        if len(report) == 0:
    ...
    def get_caption_base_fu_text_mask(self, base_report, fu_report):
        
        base_report = list(filter(lambda x: x != "", base_report))
        base_sent = " ".join(base_report)
        fu_report = list(filter(lambda x: x != "", fu_report))
        fu_sent = " ".join(fu_report)

        tokenized_base_text = self.tokenizer.tokenize(base_sent)
        tokenized_text = self.tokenizer.tokenize(fu_sent)
        
        for i in range(len(tokenized_text)):
            if self.random_erase > random.random():
                tokenized_text[i] = '[MASK]'
        
        tokenized_text = tokenized_base_text + tokenized_text
    ...
    def base_fu_get_item(self, index):
        ...
        base_reports = self.json_file['reports_sent'][index][0]
        fu_reports = self.json_file['reports_sent'][index][1]
        ...
        caps_mask, cap_len_mask = self.get_caption(base_reports)
