...
def text_to_sectionize(text):
    
    b_study_sectioned = []
    
    dict_sec = {'impression': [], 'findings':[], 'last_paragraph':[]}
    ...
    for sn in ('impression', 'findings', 'last_paragraph'):
        if sn in section_names:
            idx = list_rindex(section_names, sn)
            dict_sec[sn] = sections[idx].strip()
            b_study_sectioned.append(sections[idx].strip())
        else:
            dict_sec[sn] = None
            b_study_sectioned.append(None)
