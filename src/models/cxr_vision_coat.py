...
class CoaT(nn.Module):
    ...
    def __init__(self, 
        ...
        if not self.return_interm_layers:
            ...
            if self.parallel_depth > 0:                                  # CoaT series: Aggregate features of last three scales for classification.
                ...
                self.head = nn.Linear(embed_dims[3], num_label)
            else:
                self.head = nn.Linear(embed_dims[3], num_label)        # CoaT-Lite series: Use feature of last scale for classification.
    ...
    def reset_classifier(self, num_label, global_pool=''):
        self.num_label = num_label
        self.head = nn.Linear(self.embed_dim, num_label) if num_label > 0 else nn.Identity()
    ...
    def forward(self,x,x2):
        ...
        x2 = self.forward_impl(x2)
        out = torch.cat([x,x2],1)
        out = self.change_linear(out)
...
def _cfg_coat(url='', **kwargs):
    return {
        'url': url,
        'num_label': 1000, 'input_size': (1, 512, 512), 'pool_size': None,
...
#              num_heads=8, mlp_ratios=[4, 4, 4, 4])
