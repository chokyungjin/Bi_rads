...
class Inception3(nn.Module):
    def __init__(
        ...
        self.fc = nn.Linear(2048, num_label)
        
        self.change_linear = nn.Linear(1024, 2)
        self.disease_linear = nn.Linear(512, disease_classes)
    ...
    def forward(self, x1, x2) -> InceptionOutputs:
        x1 = self._transform_input(x1)
        x2 = self._transform_input(x2)
        
        x1, aux = self._forward(x1)
        x2, aux = self._forward(x2)
        ...
        # cat_aux = torch.cat((aux1, aux2), 1)
        # aux = self.change_linear(cat_aux)
        
        # aux_defined = self.training and self.aux_logits
        # if torch.jit.is_scripting():
        #     if not aux_defined:
        #         warnings.warn("Scripted Inception3 always returns Inception3 Tuple")
        #     return InceptionOutputs(out, aux)
        # else:
        #     return self.eager_outputs(out, aux)
