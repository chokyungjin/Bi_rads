...
from src.datamodules.components import CXRMultimodalv2, multimodal_collate_fn_v2
from src.models import cxr_text_model
...
parser.add_argument('--log_dir', type=str, default="./results/text/")
...
parser.add_argument('--bert_pretrain', type=str, default=None)
...
def test(args, test_dataloader, model, device, log_dir):
    ...
    for idx, batch in tqdm.tqdm(enumerate(test_dataloader)):
        ...
        with torch.no_grad():
            ...
            _, outputs,_ = model(caption_ids,
                                 None,
                                 attention_mask,
                                 change_labels.to(device)
                                )
...
def main(args):
    ...
    transform = {'resize': {'height': 512,
                            'width': 512}}
    dataset_config = {"max_words": 128,
                        "pretrained_tokenizer": args.bert_pretrain,
                        "d_num": 14,
    ...
    model = cxr_text_model.CXRBertForSequenceClassifier(
                                pretrained=args.bert_pretrain,
